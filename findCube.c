// Code that finds the random cube
// For simplicity, it is the same code as playGame: the arm thread and the other functions involving the arm have been kept even if they are not used
// The functions used to move the robot from one position of the stadium to another have been removed instead, since here we are obviously taking different steps
...
int main(void)
{
    ...
    //List of actions performed by the robot in order to find the random cube
    ...
    //Go forward until arriving approximately in line with the pyramid
    printf("Going forward.\n");
    set_action(FORWARD_TIMED, 1120.0);
    wait_main();
    ...
    //Go forward until reaching the edge of the stadium 
    printf("Going forward.\n");
    set_action(FORWARD, 210.0);
    wait_main();
    //Rotate 90 degrees clockwise again, then start reading with the sonar, looking for a cube
    printf("Rotating cw.\n");
    diff = computeDiff(initGyro, angle);
    set_action(ROTCW, 90.0-diff);
    wait_main();
    angle += 90.0; // update angle: robot is at 180 degrees
    calibrate(initGyro, angle);
    printf("Reading sonar.\n");
    set_action(READING, 0.0);
    wait_main();
    
    //If the distance read is lower than the distance between the robot and the end of the stadium, it means that a cube was found
    //Then, go forward until stopping in front of it
    if(checkForCube(1020.0)){
        printf("Cube found: going forward.\n");
        set_action(FORWARD, 150.0);
        wait_main(); 
    }
    //If the cube is not found, go forward until reaching the end of the stadium
    else {
        printf("Cube NOT found: going forward.\n");
        set_action(FORWARD, 180.0);
        wait_main();
        //Then, rotate 90 degrees clockwise and read again with the sonar
        printf("Rotating cw.\n");
        diff = computeDiff(initGyro, angle);
        set_action(ROTCW, 90.0-diff);
        wait_main();
        angle += 90.0; // update angle: robot is at 270 degrees (= -90 degrees)
        calibrate(initGyro, angle);
        printf("Reading sonar.\n");
        set_action(READING, 0.0);
        wait_main();
        
        //Again, if the distance is lower than the one we would read if there was no cube, it means that a cube is present
        //Then, go forward until stopping in front of it
        if(checkForCube(1020.0)){
            printf("Cube found: going forward.\n");
            set_action(FORWARD, 150.0);
        ...
        //Otherwise, go forward until reaching the edge of the stadium, rotate 90 degrees clockwise and go forward until stopping in front of the cube
        else {
            printf("Going forward.\n");
            set_action(FORWARD, 210.0);
            wait_main();
            ...
            set_action(ROTCW, 90.0-diff);
            wait_main();
            angle = 0;  // update angle: robot is at 0 degrees
            calibrate(initGyro, angle);
            printf("Cube found: going forward (3 cm).\n");
            set_action(FORWARD, 150.0);
            wait_main();
        }
    }
